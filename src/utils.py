...
def load_object(file_path):
    try: 
        ...
        raise CustomException(e,sys)
def upload_file(from_filename,bucket_name,to_filename):
    try:
        s3_resource = boto3.resource('s3')
        s3_resource.meta.client.upload_file(from_filename,bucket_name,to_filename)
    except Exception as e:
        raise(e,sys)
def download_file(bucket_name,bucket_filename,dest_filename):
    try:
...
def evaluate_models(X,y,models):
    try:
        ...
        for model in models.values():
            ...
            model_test_score = accuracy_score(y_test,y_pred_test)
